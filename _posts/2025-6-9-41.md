...
tags: [Excited State, Wavefunction Analysis, NTO, Transition Density, Multiwfn]
math: true
---

记录下使用Gaussian与Multiwfn如何做激发态分析。

打个广告，banewfn可以轻松完成激发态的波函数分析，宣传视频：

{% include embed/bilibili.html id='BV12nAYzZE51' %}
...
## 9. NBO贡献分析
可以借助NBO分析和Multiwfn的密度拟合功能将激发态密度差拟合为NBO贡献。首先，需要按[第七节](#7-密度差-density-difference)所述计算密度差。然后进行NBO计算：
```
%chk=NBO.chk
# b3lyp/def2tzvp scrf=(SMD,solvent=water) em=gd3bj pop=nboread

abs

0 1
C -0.52723700 -0.00000500 0.00005400
O 0.67708900 -0.00000600 -0.00001300
H -1.12659200 0.94368400 -0.00010700
H -1.12670200 -0.94360600 -0.00010700

$NBO plot file=HCHO $END


```
该计算将输出HCHO.31-HCHO.41等一系列NBOplot文件。在Multiwfn中载入NBO轨道，结合密度差进行拟合：
```
HCHO.31   // 载入31文件
37        // 37是载入NBO轨道
200
...
此时，屏幕上会输出NBO轨道的贡献：
```
 Orbital     8   Value:    -0.652
 Orbital     1   Value:    -0.380
 Orbital    16   Value:    -0.173
 Orbital     3   Value:    -0.144
 Orbital     4   Value:    -0.144
 Orbital    57   Value:    -0.094
 Orbital    58   Value:    -0.084
 Orbital     9   Value:    -0.072
 Orbital    26   Value:    -0.064
 Orbital     7   Value:    -0.059
 
...

 Orbital    23   Value:     0.076
 Orbital    43   Value:     0.078
 Orbital    54   Value:     0.198
 Orbital    71   Value:     1.052
 Sum of all values:       0.000
 Fitting error (definition 1):      0.2765
 Fitting error (definition 2):    0.000358

 0 Exit
 1 Output fitted grid data to fitted.cub in current folder
 2 Visualize isosurface of provided grid data
 3 Visualize isosurface of fitted grid data
 4 Visualize isosurface of difference between the provided grid data and the fitted grid data

```
可以看到，NBO8和NBO71对该跃迁贡献最大。可以观看NBO轨道确定轨道类型：

![](https://pub-ec46b9a843f44891acf04d27fddf97e0.r2.dev/2026/02/20260227180642.png)

这样看的就很清楚了，甲醛的S1跃迁主要是氧的孤对电子跃迁到pi轨道。
