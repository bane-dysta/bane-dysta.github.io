...
tags: [SNTO, PET, Charge Transfer, State Tracking, Excited State]
asset_id: 58
math: true
chem: true
---
...
{% include xyz.html 
   mode="2d" 
   src="assets/posts/58/model.mol"  
   mol=mol_text 
   width=500 height=350 %}
（硝基在Chemdoodle里不好画，总想自动补氢，这里干脆整成双键了）
...
此处，我们可以利用SNTO来确定透热态势能面是怎么走的。[baneSNTO](/assets/posts/58/baneSNTO.tar.xz)是一个专为计算SNTO而设计的程序，可以批量计算某电子态与下一个结构的每个电子态的SNTO。以下我们研究透热态LE的走向。首先生成每个态NTO文件：
```
...
```
重复此步骤，我们最终可以得到一条透热LE态的轨迹。对FC点的绝热S2做同样处理，我们也可以得到CT态的轨迹。但是上述流程有点麻烦，笔者用GPT5.2写了个脚本[diabatic.sh](/assets/posts/58/diabatic.sh)来完这些成重复工作：
```
