...
- -p [number]; --gridpoints [number]：设置数据点数量，默认为40000，n值越大，生成的箭头和等值面约平滑，但相应计算成本会增加。
- -s; --smoothisosurface：平滑化，避免边缘棱角。
...
```
一共三块object，每个块控制一个试图。translate后面的坐标控制每个对象的位置，改大一些可以增加间距。(Z坐标应当保持为0)

展示一个最终效果：

![](https://pub-ec46b9a843f44891acf04d27fddf97e0.r2.dev/2025/04/AICD.png)

顺时针流动为芳香性；逆时针流动为反芳香性。
