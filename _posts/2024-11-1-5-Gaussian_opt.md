...
tags: [Gaussian, Geometry Optimization, Frequency Analysis, Energy Calculation, Tutorial]
---
...
参考：[Gaussian的安装方法及运行时的相关问题](http://sobereva.com/439)
...
### 1.5 使用笔者的脚本
笔者曾用RDkit做了一个简易的[构象搜索程序](/assets/posts/5/confsearch.zip)，对小型体系可以产生较为可靠的3d构象。双击运行，输入smiles字符串进行转换。反馈可以去笔者在公社发的[贴子](http://bbs.keinsci.com/forum.php?mod=viewthread&tid=59528&fromuid=63020)。
...
# opt freq wB97XD/def2SVP           // 关键词行（route）
...
```
其中：
- `%`开头的行被称为[link0 commands](https://gaussian.com/link0/)，用于指定资源分配等。其中%chk指向的文件是波函数文件，对后处理尤为重要，需要小心保存。
- `#`开头的行被称为[route](https://gaussian.com/keywords)，俗称关键词行。该行指定执行的计算任务。
- title行：随便写，但是不能留空，不能有中文
- 电荷与自旋多重度：需要根据体系进行设置
- 几何坐标：通常是建模后自动保存的

> 对于真的想要做计算的读者，推荐熟悉一下输入文件的结构。gview的图形界面非常复杂，只适合萌新上手使用。在正式进行计算时，笔者强烈推荐手写输入文件。

### 2.1 电荷与自旋多重度
虽然几何结构已经有了，但是程序自动给出的电荷和自旋不一定是对的，需要手动检查。

电荷就是体系的总电荷数。需要注意的是，高斯不需要用户指定电荷分布在哪里。原因涉及到量子化学计算的原理，比较复杂。总之读者只需记住：实验化学家所熟悉的Lewis式形式电荷不等同于量子化学中的原子电荷，一般量化计算的对象是所有共振式共同描述的真实体系，而非某个共振极限式。
...
### 2.2 基组的选择
基组是描述电子在空间中分布的基函数集，用来组合分子体系的电子波函数。笔者常用的基组系列有：
...
报错是很正常的。笔者初学时，花了一个星期才写出来第一个能从头到尾不报错跑完整的计算任务。哪怕是现在，笔者也依然经常写错关键词。对于计算时遇到的报错问题，可以在这个[帖子](http://bbs.keinsci.com/forum.php?mod=viewthread&tid=4829&fromuid=63020)里查找解决办法，或按照帖子所述提取关键信息发到论坛或者群里求助别人。

这里给出一个笔者自用的比较省事的工具：[g16loop](https://bane-dysta.top/software/g16loop/)。该程序是g16的套壳脚本，在你的任务失败时会自动处理一些非常常见的错误，同时还会把很可能包含报错的尾部日志保存成tail.txt，如果出现无法处理的报错可以拿着这个咨询大师。
