...
tags: [SDNTO, SNTO, Excited State, State Tracking, Gaussian]
asset_id: 55
math: true
mermaid: true
---

在计算光化学中，优化分子的激发态几何结构是理解光化学反应机理的关键步骤。然而，这个看似简单的任务实际上充满挑战。想象一下这样的场景：想要优化某个分子的S2，但在优化过程中，随着分子几何构型的变化，原本的S2态可能会变成S1，甚至S3。如果不加以追踪，优化算法很容易丢失目标，最终得到的不是你想要的那个电子态的最小值，而是另一个完全不同性质的态。这就是激发态追踪（state tracking）要解决的核心问题：**如何在几何优化过程中始终跟随具有相同电子性质的态，即所谓的"diabatic态"？**
...
## Starte
利用Gaussian接口，笔者写了个SDNTO程序(此处已经不是最速下降算法，所以大概该叫gau-SNTO?)，命名为[starte](/assets/posts/55/starte_static.7z)，这是start-stare-state三个词的结合，寓意为‘开始盯着这个态’。工作流程：
...
| `banesnto` | logical | `.T.` | 使用baneSNTO进行重叠积分计算，而不是调用Multiwfn |
...
inttype=2 sntorange=2 nadd=4 sdnto=.t.
