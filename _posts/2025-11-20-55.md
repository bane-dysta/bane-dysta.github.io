...
| `inttype` | integer | `1` | NTO 重叠积分方案：<br/>`1` - 直接重叠积分（原始论文方法）<br/>`2` - 波函数范数的重叠积分 |
| `debug` | logical | `.F.` | 启用调试输出模式 |
...
| `method` | string | 功能（如 `b3lyp`, `cam-b3lyp`） |
| `basis` | string | 基组（如 `6-31g`, `6-311+g(d,p)`） |
| `root` | integer | 要跟踪的根态号 |
| `nadd` | integer | 计算 nstate 时的偏移量（默认 `2`） |
