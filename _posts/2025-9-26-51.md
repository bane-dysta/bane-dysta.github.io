...
tags: [Electronic Coupling, Charge Transfer, Excited State]
math: true
---
...
## banetask
从2.46.0版本开始，banetask增加了计算电荷转移积分的方法，并可以从一个结构和一个输入文件直接计算出最终的电荷转移积分，计算后端可以选择Gaussian和ORCA。示例btc文件(Gaussian后端)：
```bt
autorun: true
totcore: 32
totmem: 96000
...
$opt
  %source origin
  %keywords opt freq [func] [basis] 5D

$dimersp
  %source opt
  %keywords nosymm [func] [basis] 5D iop(3/33=1)

@foreach? frag
$part_[frag.name]
  %source opt
  %keywords nosymm [func] [basis] 5D
  %mod 
...
$CTI
  %source
    ...
    system dimersp
  %action ct.integral
    orbitals [homo-2 homo-1 homo lumo lumo+1]
```
