...
在这个过程中，为了衡量跃迁的可能性，我们引入**跃迁偶极矩**来描述电子对跃迁的贡献：

$$
I_{i \leftarrow j} \propto |\mu_{ij}^e|^2
$$

其中，跃迁偶极矩$\mu_{ij}^e$的数学表示为：

$$
\mu_{ij}^e = |\langle \psi_i|\hat{\mu}|\psi_j \rangle|^2
$$

式中: 
- $\psi_i$、$\psi_j$是初态与末态的电子波函数；
- $\hat{\mu}$​ 是电偶极矩算符，可以写为：$\hat{\mu} = -e\sum_j \mathbf{r}_j$


## 3 振动能级
我们刚刚假设分子总是处于势能面极小点结构来理解垂直激发，而在实际过程中这个假设并不成立。事实上由于不确定性原理，原子不能同时具有精确的位置和动量，分子总是在不断振动的。因此，在某一时刻下，一个宏观体系中必然含有处于不同振动态的分子。在谐振模型下，振动态的能量是离散的，形成了不同的振动能级。将振动能级绘制在势能面上，可得到下图：
...
- $\langle \chi_i \|\chi_j \rangle$表示态$i$与态$j$波函数的重叠积分
