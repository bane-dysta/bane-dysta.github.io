...
我们在第一节提到过，电子态决定于不同的电子结构。可想而知，当体系从S0态跃迁至S1态时，电子分布会产生改变。我们可以用用**跃迁密度(Transition Density, TD)**来描述这种改变，其定义为：
...
$$
\mu_{ij}^e = |\langle \psi_i|\hat{\mu^e}|\psi_j \rangle|^2
$$
...
由$(5.1)$ - $(5.3)$式可知，辐射跃迁速率的主要影响因素有两个：
- 两个态之间的能量差  
  表观上，辐射跃迁速率$A_{ji}$与两态之间的能量差$E$的平方成正比，而实际上由于振子强度也与$E$成正比关系，实际上$A_{ji}$是与$E$的三次方成正比的。因此，两个态之间的能量差增加会极大地有利于辐射跃迁，这也是吸收光谱上短波区出现强吸收峰的原因所在。

- 振子强度$f_{ij}$  
  由$f_{ij}$表达式可知，变量有两态之间的能量差和跃迁偶极矩。跃迁偶极矩已在第2节中介绍过。
