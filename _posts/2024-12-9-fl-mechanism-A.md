...
由上图，我们看到从激发态回到基态的通道还包括一类不发射光子的跃迁过程。这类跃迁被称为非辐射跃迁，特征是体系会通过非光子途径将激发态的能量耗散到周围环境中，回到基态。非辐射跃迁速率可以用费米黄金定则(Fermi's Golden Rule)来计算。

设初态为 $|\psi_i\rangle$，末态为 $|\psi_f\rangle$，对应能量为 $E_i$ 和 $E_f$，则：
