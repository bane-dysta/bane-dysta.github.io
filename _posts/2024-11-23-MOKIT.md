...
tags: [MOKIT, GAMESS, PySCF, Installation, HPC]
---
...
#### 旧修改版的更新
MOKIT在最近的某次更新里增加了对GAMESS的MRSF-TDDFT功能的修改，使用旧修改脚本编译的可以如此生成新的gamess.01.x：
```
cd ~/apprepo/gamess
cd source
...
### 移植
笔者发现使用ifort和mkl在CentOS7上编译的gamess可以直接移植到Ubuntu22.04，全部测试通过，可以看出编译好的gamess绝大部分库都静态链接进去了，可移植性相当高。这些文件应该是gamess运行必须的：
```
tar -cJvf gamess.tar.xz gamess/auxdata gamess/scr gamess/tools gamess/ddikick.x gamess/gamess.01.x gamess/gms-files.csh gamess/runall gamess/rungms gamess/*.inp gamess/tests/standard
```
打包之后在另一台机器上可以直接解压使用
```
#!/bin/bash

tar -xJf gamess.tar.xz
mv gamess ~/apprepo

cd ~/apprepo/gamess
cp gamess.01.x gamess.00.x 

./runall 01
mv *.log tests/standard
cd tests/standard
./checktst
```

### 让gamess使用conda安装的mkl
安装mkl：
```
conda install -c conda-forge mkl mkl-include
```
config生成install.info之后，去看看`GMS_MATHLIB_PATH`路径下有没有`libmkl_rt.so`。如果没有，改成正确的配置
```
GMS_MATHLIB_PATH=/apps/miniconda/envs/mokit/lib
```
直接编译，大概率通过不了，因为conda安装的mkl只有`libmkl_rt.so`，但gamess的lked非得要静态库。想要用conda装的mkl，需要把lked的这一段
```
      case mkl:
...
            default:
               echo "unknown version number for MKL/linux64" >> /dev/stderr
               echo "please edit 'lked' to select the right 64 bit MKL libraries." >> /dev/stderr
               exit
               breaksw
         endsw
         breaksw
...
            set mpath=$GMS_MATHLIB_PATH
            
            echo "Linking against Conda MKL runtime library..."
            set MATHLIBS="-L$mpath -lmkl_rt -lpthread -lm -ldl"
            
            # 如果是 OpenMP 并行版，可能需要额外链接 GOMP (通常包含在 gcc libs 里)
            if ($GMS_OPENMP == true) set MATHLIBS="$MATHLIBS -lgomp"
      
            breaksw
```
同时，在运行gamess之前，还要把`MKL_INTERFACE_LAYER`设对
```
export MKL_INTERFACE_LAYER=ilp64,GNU
```
